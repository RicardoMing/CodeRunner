if exists('b:current_syntax') && b:current_syntax ==# 'CodeRunner'
    finish
endif
let b:current_syntax = 'CodeRunner'
syntax case ignore
...
function! s:highlight_shell_cmd() abort
    ...
    syntax match CodeRunner__Output_Shellcmd_Conceal
                \ contained conceal    '\e\[[0-9;]*m'
                \ containedin=CodeRunner__Output_Shellcmd
    ...
    for [key, highlight] in items(highlight_table)
        let syntax_name = 'CodeRunner__Output_Shellcmd_Color'
                    \ . substitute(key, ';', '_', 'g')
        let syntax_command = printf('start=+\e\[0\?%sm+ end=+\ze\e[\[0*m]\|$+ ' .
                    \ 'contains=CodeRunner__Output_Shellcmd_Conceal ' .
                    \ 'containedin=CodeRunner__Output_Shellcmd oneline', key)
